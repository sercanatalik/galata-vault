...
use galata_vault::{Actor, Api, Error, Expected, SecretSet, Vault};
use galata_vault_keys::TokenKeys;
use galata_vault_proto::children::{RESERVED_PREFIX, is_reserved_name};
...
use crate::fsutil::create_private;
...
pub fn run(
    ...
    // the child and exits with its status (`crate::util::replace_process`).
    let err = crate::util::replace_process(cmd);
