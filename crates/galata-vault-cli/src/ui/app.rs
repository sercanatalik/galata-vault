...
use galata_vault::owner::{Environment, Owner};
use galata_vault::{Error, Expect};
...
use crate::secrets::{conflict_text, refuse_reserved};
use crate::util::{fmt_time, now, parse_ttl};
...
fn clock() -> String {
    let (_, _, _, h, m, s) = galata_vault_proto::time::utc(now());
...
impl App {
    ...
    fn route(&mut self, req: &Req) -> Resp {
        ...
        match result {
            Ok(r) => r,
            Err(e) => {
                let message = crate::render_error(&e, &self.branding);
    ...
    fn perform(&mut self, p: Pending) {
        let outcome = self
            .run_op(&p)
            .unwrap_or_else(|e| Outcome::Failed(crate::render_error(&e, &self.branding)));
        match &outcome {
    ...
    fn hand_over(&mut self, req: &Req) -> anyhow::Result<Resp> {
        ...
        match req.path.as_str() {
            ...
            "/api/handover/save" => {
                ...
                match crate::fsutil::create_private(&path, line.as_bytes(), false) {
                    Ok(()) => {
                        self.event(
                            "save",
                            &env,
