...
use galata_vault_proto::api::Limits;
use galata_vault_proto::audit::Actor;
use galata_vault_proto::children::ChildMode;
...
fn token_self(server: &Server, keys: &TokenKeys) -> galata_vault_proto::api::TokenSelf {
    let pq = "/v1/tokens/self";
    let auth = keys
        .sign_request(
            &galata_vault_proto::sig::SignedRequest::new("GET", pq, b""),
            now(),
        )
        .to_header_value();
...
fn mcp_setup_maps_every_environment_to_its_own_meta_token() {
    use galata_vault_proto::api::Scope;
    use galata_vault_proto::mcp::McpConfig;
...
fn a_copied_read_key_opens_nothing_after_revoke_rotate() {
    use galata_vault_seal::{EnvelopeContext, SealError, open_value};
...
fn a_dump_of_every_server_artifact_holds_no_plaintext_paths_or_keys() {
    use galata_vault_proto::codec::{TokenString, decode_node_key};
...
fn a_copied_config_key_opens_nothing_after_revoke_rotate() {
    use galata_vault_seal::{EnvelopeContext, SealError, open_config};
    use galata_vault_store::RecordKind;
...
fn kits_and_tokens_of_another_version_are_refused() {
    let server = Server::start();
    let gv = Gv::new(&server);
    let v1_key = galata_vault_proto::codec::encode_checked("gvk2_", &[5u8; 32]);
    ...
    let v1_token = galata_vault_proto::codec::encode_checked("gvt2_", &[3u8; 48]);
    let out = gv.run_with(
        &["get", "X"],
        b"",
