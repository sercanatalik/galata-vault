...
use galata_vault_keys::{OwnerKeys, TokenKeys};
...
use galata_vault_proto::ids::{NameHmac, TokenId};
...
pub fn records_path(kind: RecordKind) -> &'static str {
    match kind {
        ...
        _ => unreachable!(
            "record kind {} has no collection in this client",
            kind.code()
        ),
    }
}
    ...
    pub fn call(
        ...
        let authorization: Option<Zeroizing<String>> = match auth {
            ...
            Auth::Owner(owner) => Some(Zeroizing::new(
                owner.sign_request(&signed, crate::now()).to_header_value(),
            )),
            Auth::Token(token) => Some(Zeroizing::new(
                token.sign_request(&signed, crate::now()).to_header_value(),
            )),
        };
        let request = Request {
            method,
...
mod tests {
    ...
    impl Transport for Canned {
        fn send(&self, _: Request<'_>) -> Result<crate::transport::Response, TransportError> {
            self.calls.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            Ok(crate::transport::Response::new(self.status, self.body))
        }
    }
