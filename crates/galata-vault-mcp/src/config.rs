...
use galata_vault_proto::mcp::{MCP_CONFIG_FILE, McpEntry};
use galata_vault_proto::path::EnvPath;
use galata_vault_proto::url::validate_server_url;
...
fn holds_node_key(v: &toml::Value) -> bool {
    match v {
        toml::Value::String(s) => s.contains(galata_vault_proto::codec::KEY_PREFIX),
...
mod tests {
    ...
    fn node_keys_modes_and_bad_tokens_are_refused_without_quoting() {
        let dir = tempfile::tempdir().unwrap();
        let key = galata_vault_keys::NodeKey::generate().encode();
        let t = token();
        ...
        let other = galata_vault_proto::codec::encode_checked("gvt2_", &[3u8; 48]);
