...
use crate::audit::{Actor, AuditRow, ChainHead, RowStop};
use crate::descriptor::SignedDescriptor;
use crate::frame::{frame, label};
use crate::ids::{B64, Hash32, Key32, NameHmac, Sig64, TokenId, VaultId};
use crate::integrity::{IntegrityError, verify_ed25519};
use crate::tolerant::{Tolerant, Vocabulary};
...
impl Formats {
    ...
    pub const CURRENT: Formats = Formats {
        descriptor: crate::descriptor::DESCRIPTOR_VERSION as u32,
        bundle: BUNDLE_VERSION as u32,
        envelope: crate::record::ENVELOPE_VERSION as u32,
        children: crate::children::CHILDREN_RECORD_VERSION,
        audit_row: crate::audit::ROW_FORMAT as u32,
        request_signature: 1,
    };
}
...
impl<'de> Deserialize<'de> for AuditPage {
    fn deserialize<D: serde::Deserializer<'de>>(d: D) -> Result<Self, D::Error> {
        ...
        let (rows, stop) = crate::audit::read_rows(raw.rows).map_err(serde::de::Error::custom)?;
        Ok(AuditPage {
            rows,
            head: raw.head,
            stop,
...
mod tests {
    use super::*;
    use crate::audit::{AuditAction, AuditEvent, AuditResult, GENESIS};
    ...
    fn every_response_tolerates_an_unknown_field() {
        ...
        check(crate::children::ChildrenBlob::new(1, B64(vec![1]), sig));
        ...
        let row = crate::audit::AuditRow::from_event(
            1,
            5,
            GENESIS,
            AuditEvent::simple(
