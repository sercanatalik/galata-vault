...
use galata_vault_keys::{ConfigSecret, VaultSecret};
use galata_vault_proto::api::AGE_HEADER;
use galata_vault_proto::ids::{Key32, VaultId};
use galata_vault_proto::record::{ENVELOPE_VERSION, RecordKind};
use zeroize::Zeroizing;

use crate::SealError;
use crate::keys::{config_identity, identity, recipient};
...
pub(crate) fn decode_envelope(bytes: &[u8]) -> Result<Decoded, SealError> {
    ...
    let (format, rest) = match kind {
        ...
        _ => return Err(EnvelopeFault::UnknownKind(kind_code).into()),
    };
...
mod tests {
    use super::*;
    use crate::{new_config_keypair, new_vault_keypair};
