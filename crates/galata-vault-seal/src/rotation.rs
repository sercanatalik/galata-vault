...
use galata_vault_keys::{FullBundle, OwnerKeys, seal_for_scope, seal_owner_bundle};
...
use crate::SealError;
use crate::record::{Writer, open_config_record, open_secret};
...
mod tests {
    ...
    use crate::record::{WrittenRecord, verify_version};
    use galata_vault_keys::{NodeKey, TokenKeys};
    use galata_vault_proto::api::{Limits, Scope, TokenSummary};
    ...
    fn a_forged_record_is_refused_not_laundered() {
        ...
        forged[0].sig = galata_vault_proto::ids::Sig64([0; 64]);
        assert!(
            build_rotation(
                &f.owner,
                &f.status,
