...
impl Harness {
    ...
    pub fn versions(&self, v: &Vault, kind: RecordKind) -> Vec<SecretVersion> {
        let kind = match kind {
            RecordKind::Secret => galata_vault_store::RecordKind::Secret,
            RecordKind::Config => galata_vault_store::RecordKind::Config,
