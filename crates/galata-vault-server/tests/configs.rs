...
use galata_vault_proto::api::{Scope, SecretList, SecretVersion};
use galata_vault_proto::record::RecordKind;
use galata_vault_seal::{ConfigFormat, Writer, build_rotation, open_config_record, open_secret};
...
async fn a_full_vault_rotates_within_the_derived_body_limit() {
    let limits =
        galata_vault_proto::api::Limits::new(4, 16 * 1024, 3, 96 * 1024, 6, 2, 32 * 1024, 3);
