...
//! (`#[command(flatten)] Vault(crate::cli::Command)`) and dispatch it with
//! [`crate::cli::dispatch`]; [`Cli`] adds the global `--env` and takes its name
...
impl Cli {
    ...
    /// ([`crate::REVIEW_STATUS`]), whatever the branding: a branded
    ...
    pub fn command_for(branding: &Branding) -> clap::Command {
        <Cli as CommandFactory>::command()
            ...
            .after_help(crate::REVIEW_STATUS)
            .mut_arg("env", |a| {
                a.help(format!(
...
impl From<ConfigFormatArg> for crate::ConfigFormat {
