...
use crate::client::transport::{Method, Request, Response, Transport, TransportError};
...
/// ```no_run
/// use std::time::Duration;
/// use galata_vault::client::ClientBuilder;
///
...
///     .build()?;
/// # let _ = api;
/// # Ok::<(), galata_vault::client::BuildError>(())
/// ```
#[derive(Clone)]
...
impl ClientBuilder {
    ...
    pub fn build_transport(&self) -> Result<HttpTransport, BuildError> {
        let server = crate::proto::url::validate_server_url(&self.server)
