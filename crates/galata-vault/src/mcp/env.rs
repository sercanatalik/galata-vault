...
use crate::client::{Api, ApiError, Auth, ClientBuilder};
use crate::keys::{KeyError, NameContext, NameKey, TokenKeys};
use crate::proto::api::{Scope, TokenSelf, VaultStatus};
use crate::proto::audit::{AuditRow, RowStop, verify_rows};
use crate::proto::children::is_reserved_name;
use crate::proto::descriptor::Descriptor;
use crate::proto::ids::{Hash32, NameHmac, TokenId};
use crate::proto::path::EnvPath;
use crate::proto::record::{RecordContext, RecordKind};
use anyhow::bail;

use crate::mcp::config::Entry;
...
impl Env {
    ...
    pub fn audit(&self) -> Result<(Vec<AuditRow>, AuditVerdict), String> {
        ...
        let (head, stop) = loop {
            ...
            if page.stop.is_some() || n < crate::client::AUDIT_PAGE {
