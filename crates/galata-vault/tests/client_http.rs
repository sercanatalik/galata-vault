...
use galata_vault::client::{ApiError, Auth, ClientBuilder, Method};
...
fn serve_tls() -> (u16, Arc<AtomicUsize>, Arc<AtomicUsize>) {
    ...
    let chain = vec![CertificateDer::from(
        include_bytes!("client_certs/server.der").to_vec(),
    )];
    let key = PrivateKeyDer::Pkcs8(PrivatePkcs8KeyDer::from(
        include_bytes!("client_certs/server.key.der").to_vec(),
    ));
    let config = Arc::new(
...
fn a_private_ca_is_trusted_only_when_given() {
    ...
    let ca = include_bytes!("client_certs/ca.der").to_vec();
...
fn child_proxy() {
    ...
    let get = |api: &galata_vault::client::Api| {
        api.call(Method::Get, "/v1/vault", None, Auth::None, None)
    };
