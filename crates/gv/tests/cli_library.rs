...
#[path = "../../galata-vault/examples/branded.rs"]
#[allow(dead_code)]
mod branded;
...
use galata_vault::cli::{CapturedOutput, Context, ScriptedPrompter};
...
fn child_branded() {
    ...
    let refused = galata_vault::cli::render_error(&e, &branded::ACME);
    assert!(
...
fn the_command_tree_flattens() {
    ...
    let gv = galata_vault::cli::Cli::command_for(&galata_vault::cli::Branding::GV);
...
fn help_ends_with_the_review_status() {
    ...
    let mut gv = galata_vault::cli::Cli::command_for(&galata_vault::cli::Branding::GV);
